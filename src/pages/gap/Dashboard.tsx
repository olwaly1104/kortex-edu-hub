...
export default function GapDashboard() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      ...
      {/* Em Atraso — professional table matching app pattern */}
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-semibold text-foreground leading-tight">Solicitações em Atraso</h2>
              <p className="text-[11px] text-muted-foreground">Prazo SLA ultrapassado — requerem acção imediata</p>
            </div>
            {solicitacoesEmAtraso.length > 0 && (
              <Badge variant="outline" className="ml-2 text-[10px] bg-destructive/10 text-destructive border-destructive/20">
                {solicitacoesEmAtraso.length}
              </Badge>
            )}
          </div>
          <Link to="/gap/solicitacoes" className="text-[11px] font-medium text-primary hover:underline inline-flex items-center gap-1">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/30">
                  ...
                  <th className="text-left p-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">Destino</th>
                  <th className="text-center p-3 font-medium text-muted-foreground text-xs uppercase tracking-wider whitespace-nowrap">Prazo</th>
              ...
              <tbody>
                {solicitacoesEmAtraso.slice(0, 8).map(({ sol, diff, prazo }) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-3">
                        <p className="font-medium text-foreground leading-tight">{tCfg?.label ?? sol.tipo}</p>
                        <p className="text-[11px] text-muted-foreground tabular-nums mt-0.5">{sol.id}</p>
                      </td>
                      <td className="p-3">
                        <p className="text-xs text-foreground leading-tight">{sol.estudante}</p>
                        <p className="text-[11px] text-muted-foreground tabular-nums mt-0.5">{sol.matricula}</p>
                      </td>
                      <td className="p-3">
                        <Badge variant="outline" className={cn("text-[10px]", dCfg.color)}>{dCfg.label}</Badge>
                      </td>
