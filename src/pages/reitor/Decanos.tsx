...
import { reitorDecanosDetail, reitorFaculties } from "@/data/institutionData";
...
export default function ReitorDecanos() {
  ...
  const getEstado = (d: typeof reitorDecanosDetail[0]) =>
    (d.presenca < 83 || d.taxaEntrega < 82 || d.mediaGeral < 13) ? "risco"
    : (d.presenca >= 88 && d.taxaEntrega >= 90 && d.mediaGeral >= 14) ? "excelente"
    : "normal";

  const filtered = useMemo(() => {
    let list = reitorDecanosDetail
      .filter(d => !search || d.name.toLowerCase().includes(search.toLowerCase()) || d.faculty.toLowerCase().includes(search.toLowerCase()))
  ...
  const totalDecanos = reitorDecanosDetail.length;
  const presencaGeral = Math.round(reitorDecanosDetail.reduce((s, d) => s + d.presenca, 0) / totalDecanos);
  const taxaEntregaGeral = Math.round(reitorDecanosDetail.reduce((s, d) => s + d.taxaEntrega, 0) / totalDecanos);
  const mediaGeral = +(reitorDecanosDetail.reduce((s, d) => s + d.mediaGeral, 0) / totalDecanos).toFixed(1);

  const resetAll = () => { setFilterStatus("todos"); setSortField(null); setSortDir("desc"); setSearch(""); };

  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><UserCog className="w-6 h-6 text-primary" /> Decanos da Universidade</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Total Decanos</span>
          </div>
          <p className="text-2xl font-bold text-foreground">{totalDecanos}</p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
      ...
      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        <div className="flex gap-2 items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Pesquisar decano..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9" />
          </div>
          ...
          <tbody>{filtered.map(d => {
            ...
            return (
              <tr key={d.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                ...
                <td className="p-3 text-xs">{facultyIdMap[d.faculty] ? <Link to={`/reitor/faculdades/${facultyIdMap[d.faculty]}`} className="text-primary hover:underline">{d.faculty}</Link> : <span className="text-muted-foreground">{d.faculty}</span>}</td>
                <td className="p-3 text-center font-medium text-foreground">{d.cursosTotal}</td>
                <td className="p-3 text-center font-medium text-foreground">{d.estudantesTotal.toLocaleString()}</td>
                <td className="p-3 text-center">{d.docentesTotal}</td>
                <td className="p-3 text-center"><span className={d.presenca >= 85 ? "text-accent font-medium" : "text-destructive font-medium"}>{d.presenca}%</span></td>
                <td className="p-3 text-center"><span className={d.taxaEntrega >= 85 ? "text-accent font-medium" : "text-destructive font-medium"}>{d.taxaEntrega}%</span></td>
                <td className="p-3 text-center"><span className={`font-medium ${d.taxaAprovacao >= 80 ? "text-accent" : d.taxaAprovacao >= 70 ? "text-foreground" : "text-destructive"}`}>{d.taxaAprovacao}%</span></td>
                <td className="p-3 text-center"><span className={d.mediaGeral >= 13 ? "text-accent font-bold" : "text-destructive font-bold"}>{d.mediaGeral}</span></td>
