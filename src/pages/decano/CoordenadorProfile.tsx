...
import { decanoCoordenadores, decanoFaculty } from "@/data/institutionData";
...
export default function DecanoCoordenadorProfile() {
  const { coordenadorId } = useParams();
  const navigate = useNavigate();
  const coord = decanoCoordenadores.find(c => c.id === coordenadorId);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap mt-1">
              <Badge variant="outline" className="text-[10px] gap-1"><GraduationCap className="w-3 h-3" /> {coord.courseCode}</Badge>
              <Badge variant="outline" className="text-[10px] gap-1"><BookOpen className="w-3 h-3" /> {decanoFaculty.name}</Badge>
              <Badge variant="outline" className="text-[10px]">Departamento de {coord.department}</Badge>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {[
            ...
            { icon: Building2, label: "Departamento", value: coord.department, color: "bg-primary/10 text-primary" },
          ].map(info => (
      ...
      <Card className="overflow-hidden">
        ...
        <div className="divide-y divide-border">
          <div className="flex items-center justify-between px-5 py-3.5">
            ...
            <p className="text-sm font-semibold text-foreground">{coord.course}</p>
          </div>
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
          ...
          <div className="flex items-center justify-between px-5 py-3.5">
            ...
            <p className="text-sm font-semibold text-foreground">{coord.estudantesTotal}</p>
          </div>
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-primary" /></div>
              <p className="text-sm text-muted-foreground">Total Docentes</p>
            </div>
            <p className="text-sm font-semibold text-foreground">{coord.docentesTotal}</p>
          </div>
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-semibold text-foreground">{coord.cadeirasTotal}</p>
          </div>
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-4 h-4 text-primary" /></div>
            ...
            <p className="text-sm font-semibold text-foreground">{coord.turmasTotal}</p>
          </div>
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
