...
export default function DecanoDocentes() {
  ...
  const getEstado = (d: typeof decanoDocentes[0]) =>
  ...
  const totalDocentes = decanoFaculty.totalDocentes;
  const presencaGeral = Math.round(decanoDocentes.reduce((s, d) => s + d.presenca, 0) / decanoDocentes.length);
  const taxaEntregaGeral = Math.round(decanoDocentes.reduce((s, d) => s + d.taxaEntrega, 0) / decanoDocentes.length);
  const mediaGeral = +(decanoDocentes.reduce((s, d) => s + d.mediaGeral, 0) / decanoDocentes.length).toFixed(1);
