...
export default function DecanoEstudantes() {
  ...
  const courses = decanoFaculty.courses.map(c => c.name);
  ...
  const filtered = useMemo(() => {
    let list = decanoEstudantes
      .filter(e => filterCourse === "todos" || e.course === filterCourse)
      .filter(e => {
    ...
    return list;
  }, [filterCourse, search, sortField, sortDir, filterStatus]);

  const totalEstudantes = decanoFaculty.totalEstudantes;
  const excelentes = decanoEstudantes.filter(e => e.status === "excelente").length;
  const normais = decanoEstudantes.filter(e => e.status === "normal").length;
  const emRisco = decanoEstudantes.filter(e => e.status === "risco").length;

  const resetAll = () => { setFilterStatus("todos"); setSortField(null); setSortDir("desc"); setSearch(""); setFilterCourse("todos"); };

  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Users className="w-6 h-6 text-primary" /> Estudantes da Faculdade</h1>
