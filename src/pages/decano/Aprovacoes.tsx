        ...
        <div className="flex flex-wrap gap-2">
          {mainTabs.map(t => (
            <button key={t.key} onClick={() => { setTab(t.key); setSubTab("pendentes"); setTypeFilter(null); }}
              ...
              {t.label}
              {t.count > 0 && (
                <span className={cn("ml-0.5 text-[10px] font-semibold rounded-full px-1.5 min-w-[16px] text-center",
        ...
        {filtered.length === 0 && (
          <Card className="p-12">
            <div className="text-center">
              ...
              <p className="text-muted-foreground font-medium">{subTab === "pendentes" ? "Nenhuma solicitação pendente" : "Nenhum registo no histórico"}</p>
              <p className="text-xs text-muted-foreground mt-1">{subTab === "pendentes" ? "Todos os pedidos foram processados 🎉" : "Ajuste os filtros para ver resultados"}</p>
            </div>
          </Card>
        )}
      </div>
