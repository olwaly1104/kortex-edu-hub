...
// Regenerate cadeiras consistently (same as TurmaDetail)
...
function generateLessons(cadeiraCode: string) {
  return Array.from({ length: 4 }, (_, i) => ({
    ...
    attendance: Math.floor(25 + Math.random() * 10),
    totalStudents: 34,
...
export default function DecanoCadeiraDetail() {
  const { cursoId, turmaId, cadeiraId } = useParams();
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <Link to={`/decano/cursos/${cursoId}/turma/${turmaId}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
        ...
        <TabsContent value="students" className="space-y-4">
          ...
          <div className="space-y-2">
            {filteredStudents.map(student => (
              <Card key={student.id} className={`p-4 flex items-center gap-4 border-l-[3px] hover:shadow-md transition-shadow cursor-pointer ${statusColors[student.status]}`} onClick={() => navigate(`/decano/estudantes/${student.id}`)}>
