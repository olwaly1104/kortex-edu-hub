...
export default function DecanoCoordenadores() {
  ...
  const getEstado = (c: typeof decanoCoordenadores[0]) =>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><UserCog className="w-6 h-6 text-primary" /> Coordenadores</h1>
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>{filtered.map(c => {
            ...
            return (
              <tr key={c.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => navigate(`/decano/coordenadores/${c.id}`)}>
