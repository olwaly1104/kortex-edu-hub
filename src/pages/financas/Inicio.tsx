...
import { LayoutDashboard, CalendarDays, AlertCircle, CheckSquare, Receipt } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Pendentes hoje", value: 0, icon: AlertCircle },
          { label: "Solicitações", value: 0, icon: CheckSquare },
          { label: "Agendados", value: 0, icon: CalendarDays },
          { label: "Lançamentos", value: 0, icon: Receipt },
        ].map((k) => {
          const I = k.icon;
          return (
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          <header className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-amber-600 tabular-nums">0</span>
          </header>
          <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
