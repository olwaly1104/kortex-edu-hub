...
export default function ProfessorTasks() {
  ...
  const filtered = allTarefas
    .filter(t => filterTurma === "all" || t.turmaId === filterTurma)
    .filter(t => filterStatus === "all" || t.status === filterStatus)
    .filter(t => t.title.toLowerCase().includes(searchTerm.toLowerCase()));

  const activeTasks = allTarefas.filter(t => t.status === "publicada");
  const closedTasks = allTarefas.filter(t => t.status === "encerrada");
  const pendingCorrection = allTarefas.filter(t => t.status !== "rascunho" && t.avgGrade === null && t.submissions > 0).length;
  const totalSubmissions = allTarefas.reduce((s, t) => s + t.submissions, 0);
  const totalExpected = allTarefas.filter(t => t.status !== "rascunho").reduce((s, t) => s + t.totalStudents, 0);
    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-secondary" /> Tarefas
          </h1>
          <p className="text-muted-foreground mt-1">Gerir tarefas atribuídas aos estudantes</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card className="p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">{closedTasks.length}</p>
        </Card>
        <Card className="p-4 space-y-1">
          <div className="flex items-center justify-between">
          ...
          <p className="text-2xl font-bold text-foreground">{avgSubmissionRate}%</p>
        </Card>
        <Card className="p-4 space-y-1">
          <div className="flex items-center justify-between">
          ...
          <p className={`text-2xl font-bold ${overallAvg && Number(overallAvg) >= 10 ? "text-accent" : overallAvg ? "text-destructive" : "text-muted-foreground"}`}>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0">{disc?.code}</Badge>
                    {turma && <Badge variant="outline" className="text-[10px] px-1.5 py-0">{turma.name}</Badge>}
                  </div>
                  <Badge className={`${sStyle.bg} gap-1 text-[10px]`}>
                ...
                <div className="flex items-center gap-5 text-xs text-muted-foreground mb-3 flex-wrap">
                  ...
                  {task.avgGrade === null && task.status !== "rascunho" && task.submissions > 0 && (
                ...
                {task.status !== "rascunho" && (
                  <div className="space-y-1.5">
                    ...
                    {missingCount > 0 && task.status === "publicada" && (
                      <p className="text-[11px] text-destructive/70 flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" />
                        {missingCount} estudante(s) ainda não entregaram
                      </p>
                    )}
                  </div>
                )}
