...
export default function ReitoriaFinancas() {
  const salarioBase = 650000;
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="p-5 text-center"><p className="text-2xl font-bold text-accent">{Math.round(salarioBase*0.85).toLocaleString()} Kz</p><p className="text-xs text-muted-foreground">Salário Líquido</p></Card>
        <Card className="p-5 text-center"><p className="text-2xl font-bold text-destructive">{Math.round(salarioBase*0.15).toLocaleString()} Kz</p><p className="text-xs text-muted-foreground">Descontos</p></Card>
      </div>
      ...
      <tbody>{months.map((m,i) => { const paid = i < 2; return (<tr key={m} className="border-b last:border-0 hover:bg-muted/20"><td className="p-3 font-medium text-foreground">{m} 2024</td><td className="p-3 text-center">{salarioBase.toLocaleString()} Kz</td><td className="p-3 text-center">{Math.round(salarioBase*0.85).toLocaleString()} Kz</td><td className="p-3 text-center"><Badge className={paid ? "bg-accent/10 text-accent text-[10px]" : "bg-secondary/10 text-secondary text-[10px]"}>{paid ? "Pago" : "Pendente"}</Badge></td><td className="p-3 text-center">{paid && <FileText className="w-4 h-4 text-primary mx-auto cursor-pointer" />}</td></tr>); })}</tbody></table></Card>
    </div>
  );
}
